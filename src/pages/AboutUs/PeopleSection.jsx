...
export default function PeopleSection({ people, setPeople, isOpen, onToggle, isActive, onToggleActive }) {
  return (
    <div className="w-full rounded-[12px] border border-[#E5E7EB] bg-white overflow-hidden">
      ...
      {isOpen && (
        <div className="px-6 pb-6 pt-3 flex flex-col gap-4 border-t border-[#F2F3F5] bg-[#FFFFFF]">
          {people.map((p, idx) => (
            <div key={p.id} className="p-4 rounded-[12px] border border-[#E5E7EB] bg-[#FBFCFD] flex flex-col gap-3">
              <p className="text-[14px] font-[600] text-[#0F1729]">Person {idx + 1}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={p.name}
                    onChange={(e) => setPeople(people.map((item) => (item.id === p.id ? { ...item, name: e.target.value } : item)))}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className={inputClass}
                    value={p.designation}
                    onChange={(e) => setPeople(people.map((item) => (item.id === p.id ? { ...item, designation: e.target.value } : item)))}
                  />
                </div>
              </div>
            </div>
